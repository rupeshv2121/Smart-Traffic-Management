...
interface RealisticCivilianAutoProps {
  lane: ApproachLane;
  offset: number;
...
const SPEED = 5.2; // Autos are slightly slower
...
const STOP_LINE_DISTANCE = 18; // Stop line is 4 units before crosswalk (autos need more distance)
...
const AUTO_COLORS = [
  0xFFDD00, // Yellow (most common)
  0x00AA00, // Green
  0xFFDD00, // Yellow
  0xFFAA00, // Orange-Yellow
  0xFFDD00, // Yellow
  0x33AA33, // Light Green
];

export function RealisticCivilianAuto({ lane, offset, laneIndex, getSignalForLane, emergencyYield, color }: RealisticCivilianAutoProps) {
  ...
  const id = useMemo(() => `rauto-${lane}-${laneIndex}-${offset}`, [lane, laneIndex, offset]);
  const autoColor = useMemo(() =>
    color || AUTO_COLORS[Math.abs(Math.round(offset * 5)) % AUTO_COLORS.length],
    [offset, color]
  );
  ...
  useFrame((_, dt) => {
    ...
    if (emergencyBehind) {
      ...
      const criticallyClose = distAhead < MIN_FOLLOW_DIST * 0.6;

      if (shouldStopForNonGreen && distToSignal !== null && distToSignal <= STOP_LINE_DISTANCE + 2.5) {
        targetSpeed = 0;
      }
    ...
    const adjustRate = targetSpeed < speedRef.current ? 0.4 : 0.08;
  ...
  const move = speedRef.current * dt * activeConfig.dir;
